...
def get_word_score(word, n):
    ...
    component1 = sum([SCRABBLE_LETTER_VALUES[x] for x in word])
    component2 = max(1,7*word_length-3*(n-word_length))
...
def update_hand(hand, word):
    ...
    new_hand = {}
    for x in word_dict:
        hand_copy[x] = hand_copy.get(x,0)-word_dict[x]
    for x in hand_copy:
        if hand_copy[x]>0:
            new_hand[x] = hand_copy[x]
    return new_hand
#
